...
@bolt_app.command("/correction")
def handle_correction_command(ack, body, say, client):
    ack()  # acknowledge first (within 3s)

    user_id = body.get("user_id")
    channel_id = body.get("channel_id")
    text = body.get("text", "").strip()

    # Try to get the thread_ts from the trigger message
    # Slack doesn’t send it directly, so we fetch latest message from user in channel
    parent_message = ""
    try:
        ...
        if messages:
            last_msg = messages[0]
            if "thread_ts" in last_msg:  # correction was run inside a thread
                thread_ts = last_msg["thread_ts"]

                # Fetch parent message of thread
                replies = client.conversations_replies(channel=channel_id, ts=thread_ts, inclusive=True)
                if replies.get("messages"):
                    parent_msg = replies["messages"][0]  # first = parent
                    parent_message = parent_msg.get("text", "")
    except Exception as e:
        print(f"⚠️ Could not fetch parent message: {e}")

    if not parent_message:
        say(f"<@{user_id}> ⚠️ Please use `/correction` inside a thread so I can link it to a question.")
        return

    # Save correction
    smart_tracker.save_confident_answer(parent_message, text)

    say(
        text=f"<@{user_id}> ✅ Correction saved for:\n*Q:* {parent_message}\n*A:* {text}",
        channel=channel_id,
...
#     print(f"Received Slack command: {request.form}")
#     return slack_handler.handle(request)
